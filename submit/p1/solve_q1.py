...
def fit_environment(environment, fit_window_s):
    """Fit both environmental series on a local extended window and return a smooth input."""
    ...
    diagnostics = {'method_name': 'stretched_exp',
                   'family': 'initial-value-fixed stretched exponential',
...
def run_sensitivity(n, raw_environment, fit_window_s, reference_t=None, reference_c=None):
    ...
    add('linear_input', 'Attachment-1 60 s knot values with piecewise-linear interpolation.',
        linear_t, linear_c)
    ...
    for window_s in (3600.0, 7200.0):
        ...
        add(f'fit_window_{int(window_s)}s',
            f'Initial-value-fixed stretched-exponential fit on 0–{int(window_s)} s.',
            scenario_t, scenario_c)

    for factor in (0.8, 1.2):
        scenario_t = solve_field(n, 'T', primary_environment, h_factor=factor,
                                 boundary=primary_boundary)
        add(f'h_factor_{factor:.1f}', f'Thermal convection coefficient h × {factor:.1f}.',
            scenario_t, reference_c)
    for factor in (0.8, 1.2):
        scenario_c = solve_field(n, 'C', primary_environment, hm_factor=factor,
                                 boundary=primary_boundary)
        add(f'hm_factor_{factor:.1f}', f'Effective mass-transfer coefficient h_m × {factor:.1f}.',
            reference_t, scenario_c)
    for factor in (0.8, 1.2):
        scenario_c = solve_field(n, 'C', primary_environment, d_factor=factor,
                                 boundary=primary_boundary)
        add(f'D_factor_{factor:.1f}', f'Internal moisture diffusivity D(C) × {factor:.1f}.',
            reference_t, scenario_c)
...
def write_sensitivity_report(sensitivity):
    ...
    lines = [
        ...
        '## 1. 结论与使用范围',
        '',
         f"第一问主计算采用附件 1 的 0–{int(fit['fit_window_s'])} s 数据，选择代码方法 stretched_exp（初值固定的拉伸指数函数）；求解器仅在 0–1800 s 内直接调用该函数作为边界条件。它用于平滑 60 s 记录间的输入，不改变第一问的控制方程、初值和材料参数。",
        '',
