...
p.add_argument('--root',type=Path,default=HERE.parent.parent/'questions'/'A题')
p.add_argument('--book',type=Path,default=HERE/'result4.xlsx')
a=p.parse_args()
s=json.loads((HERE/'validation.json').read_text(encoding='utf-8'))
data=json.loads((HERE/'result4_data.json').read_text(encoding='utf-8'))
for f,k in [('附件1.xlsx','environment_sha256'),('附件2.xlsx','radius_sha256')]:
    assert hashlib.sha256((a.root/'附件'/f).read_bytes()).hexdigest()==s[k]
with np.load(HERE/'result4_full_precision.npz') as full:
...
report=(HERE/'p4.md').read_text(encoding='utf-8')
...
(HERE/'validation_export.json').write_text(json.dumps(r,ensure_ascii=False,indent=2),encoding='utf-8')
