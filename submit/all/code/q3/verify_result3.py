...
p.add_argument('--book',type=Path,default=PACKAGE/'results'/'q3'/'result3.xlsx')
a=p.parse_args()
book=a.book
s=json.loads((PACKAGE/'results'/'q3'/'validation.json').read_text(encoding='utf-8'))
payload=json.loads((PACKAGE/'results'/'q3'/'result3_data.json').read_text(encoding='utf-8'))
...
with np.load(PACKAGE/'results'/'q3'/'result3_full_precision.npz') as full:
...
w.close()
table=(PACKAGE/'results'/'q3'/'结果表.md').read_text(encoding='utf-8')
assert '{{' not in table
assert any(line.startswith('|') for line in table.splitlines())
...
(PACKAGE/'results'/'q3'/'validation_export.json').write_text(json.dumps(result,ensure_ascii=False,indent=2),encoding='utf-8')
