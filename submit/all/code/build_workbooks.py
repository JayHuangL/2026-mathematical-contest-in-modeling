"""Build all four result workbooks from the JSON payloads in submit/all/results."""
...
def _write_two_sheet(question: str, template_name: str, keys: tuple[str, str]) -> None:
    ...
    book.save(folder / f"result{question[1:]}.xlsx")
    book.close()


def _write_q3() -> None:
    folder = RESULTS / "q3"
    payload = json.loads((folder / "result3_data.json").read_text(encoding="utf-8"))
    output = folder / "result3.xlsx"
...
def _write_q4() -> None:
    folder = RESULTS / "q4"
    payload = json.loads((folder / "result4_data.json").read_text(encoding="utf-8"))
    output = folder / "result4.xlsx"
