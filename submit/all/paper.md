...
主结果为：第一问 1800 s 时体积平均温度 35.169887 °C、平均干基含水率 2.293557 kg/kg；第二问 3 h 时平均温度 49.8046 °C、平均含水率 1.3824 kg/kg；第三问固定半径模型的全域达标时间为 57.6790 h；第四问采用附录4和 PCHIP 半径后，达标时间为 51.2683 h。网格、时间积分、Jacobian、含水率物理范围和水分收支检查表明数值误差已收敛，但长期边界延拓、有效传质系数、物性经验式和均匀收缩假设带来的不确定性仍明显大于离散误差。因此，文中时间均解释为给定输入和假设下的模型预测，而非精确到秒的实验结论。
...
将径向区域分成 \(N\) 个环状控制体。第 \(i\) 个控制体的内、外界面记为 \(a_i=r_{i-1/2}\)、\(b_i=r_{i+1/2}\)，节点值记为 \(T_i,C_i\)，径向厚度为 \(dr_i=b_i-a_i\)，并取 \(w_i=(b_i^2-a_i^2)/2\)。\(F^T_{i+1/2}\) 和 \(F^C_{i+1/2}\) 分别表示去掉公共因子 \(2\pi L\) 后、沿径向正方向定义的热量和含水率界面通量。导热系数在界面取相邻节点算术平均；水分通量采用 Kirchhoff 平均，相邻控制体共享同一个界面通量。中心界面为零通量，最外界面直接代入式(7)。令 \(T_f=(T_i+T_{i+1})/2\)，则

$$
D^{\mathrm K}_{i+1/2}=\int_0^1D\!\left(T_f,C_i+s(C_{i+1}-C_i)\right)\,ds,
\qquad
F^C_{i+1/2}=\frac{r_{i+1/2}}{r_{i+1}-r_i}D^{\mathrm K}_{i+1/2}(C_{i+1}-C_i).
$$

第一问的 \(D\) 与温度无关，直接略去 \(T_f\)。积分以8点 Gauss--Legendre 求积实现；这一定义在相邻含水率相等时连续退化为局部 \(D\)，且不需要除以很小的含水率差。得到的典型半离散形式为
...
对 (h,h_m,\rho c_p,k,D) 分别作 (pm20\%\) 扰动，并以

$$
S_p^Y=\frac{Y(1.2p)-Y(0.8p)}{0.4Y(p)}
$$

衡量 3 h 体积平均温度或平均含水率 (Y) 的中心无量纲敏感度。结果表明，平均含水率对有效传质系数 (h_m) 最敏感，敏感度约为 (-0.4221)，其次为内部扩散系数 (D)，约为 (-0.1478)；其余参数的绝对敏感度均不超过 0.017。平均温度对各参数的敏感度绝对值均小于 0.009，说明 3 h 时温度场已接近稳定，而含水率预测仍主要受表面对流传质和内部扩散控制。完整扰动结果保存在 results/q2/parameter_sensitivity.json。
...
有限体积离散沿用式(8)及 Kirchhoff 水分通量，但每个隐式时间步、每次非线性迭代都使用当前节点的 \(\rho\)、\(c_p\)、\(k\)、\(D\)，并在离散方程的 Jacobian 中保留 \(T\)--\(C\) 交叉导数，即温度方程对 \(C\) 以及含水率方程对 \(T\) 的偏导项。主结果采用 \(N=400\) 个固定半径区间、401 个节点，BDF 相对/绝对容差 \(2\times10^{-10}\)、\(2\times10^{-12}\)，边界变化阶段最大步长 5 s；在每秒输出点处得到 21 个径向位置的结果，其他误差控制定义与第一问相同。体积平均量按面积权重计算：
...
$$
t_*=207644.13232\ {\rm s}=57.67892564\ {\rm h}.
$$

报告烘干时间为 **57.6790 h**。报告时刻对应 207644.4000 s，最大未舍入含水率为 0.1499999224 kg/kg，位于中心；体积加权平均含水率为 0.12430514 kg/kg。
...
| 57.6790 | 0.1500 | 0.1477 | 0.1402 | 0.1249 | 0.0525 |
...
附件2每隔 1800 s 给出一个半径记录，共 145 个点，时间范围为 0--259200 s。原始散点见图12：半径在前期下降较快，随后逐渐稳定在约 1.20 cm。第四题的控制方程和边界条件会在每个时间步用到 \(R(t)^{-2}\) 和 \(R(t)^{-1}\)，因此需要把离散半径记录转成连续函数；这个函数既要贴合观测，也不能产生半径回升或超出观测范围的现象。
...
其中 \(\mathcal I_{\mathrm{PCHIP}}\) 表示由附件2记录构造的保形分段三次插值。主计算的结束时刻为 51.2683 h，小于 72 h，因此实际没有使用末端保持规则。方法比较图见图13，左图为完整观测区间，右图放大前 24 h 以显示不同插值曲线的细小差别。
...
以相邻节点中点构造控制体，内部通量形式与式(8)相同；水分界面仍在算术平均温度下沿 \(U_i\) 到 \(U_{i+1}\) 作 Kirchhoff 积分。积分后的蓄积项分别含有 \(R(t)^2w_i\rho_i c_{p,i}\) 和 \(R(t)^2w_i\)，其中 \(\rho_i,c_{p,i}\) 由第 \(i\) 个节点的 \(U_i\) 代入附录4物性得到；表面热、湿交换项分别含有 \(R(t)h(T_a-\Theta_N)\) 和 \(R(t)h_m(C_a-U_N)\)。因此半径对内部扩散和边界交换的影响会同时进入离散方程。时间上使用隐式 BDF，环境记录点、半径记录点和每 6 h 检查点作为分段点；环境变化阶段最大步长 5 s，稳定阶段最大步长 300 s。
...
使用全域事件 \(M(t)=\max_i U_i(t)\) 和式(16)的严格阈值，得到未舍入事件时刻 \(184565.591185\ \mathrm s=51.26821977\ \mathrm h\)。向上保留四位小数并继续积分后，报告烘干时间为 **51.2683 h**；报告时刻半径 1.2000 cm，全域最大含水率 0.1499998523 kg/kg，平均含水率 0.1183957329 kg/kg。
...
并考察其绝对值的最大值：第一问、第二问分别约为 \(9.4\times10^{-16}\) 和 \(4.2\times10^{-15}\ \mathrm{kg/kg}\)；第三问为 \(6.2\times10^{-15}\ \mathrm{kg/kg}\)；第四问为 \(7.1\times10^{-15}\ \mathrm{kg/kg}\)。第四问的收支使用材料坐标和干物质基准，不能把 \(\int C\,\mathrm dV\) 直接当作水质量。这些结果验证了代码中界面通量、边界项和面积权重的一致性，但不等价于真实实验验证。
...
第三问长期边界敏感性大于 BDF 时间误差：将温度和水分分别改为固定 50 °C、0.05 kg/kg 或采用最后一小时均值，会使达标时间分别缩短约 0.2055 h 和 0.2038 h；因此 57.6790 h 是独立阶段边界假设下的预测。第四问中，附录4固定半径对照得到约 130.3588 h，而主方案为 51.2683 h；附录3收缩半径对照得到 25.3120 h，说明物性关系比单独的半径插值更能改变时间尺度。线性半径插值相对 PCHIP 只改变约 0.003538 h，将稳定尾段改为 50 °C、0.05 kg/kg 则改变约 -0.179337 h。
