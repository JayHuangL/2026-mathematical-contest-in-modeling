...
附件 1 每隔 60 s 给出烘房温度和环境水分浓度，记录范围为 0--14400 s。图 1 只绘制原始观测散点，不叠加任何拟合曲线，先用于判断数据形态：温度和环境水分浓度均在早期快速上升，随后逐步进入平台区；因此第一问需要在 0--1800 s 内构造连续边界，第二至第四问则需要将同一边界处理延伸到完整观测区间并处理稳定尾段。
...
第二问及其后的第三、第四问沿用第一问定义的 RMSE（式 (1)）、粗糙度（式 (2)）和六类候选方法，并使用其中选定的拉伸指数函数（式 (3)）；但拟合窗口覆盖附件 1 的完整 0--14400 s。这里的“全窗口拟合 RMSE”是指在全部 0--14400 s 观测点上计算的拟合残差 RMSE，不是第一问的时间留出 RMSE。分界点之前直接使用这一全数据拟合边界；只有在稳定分界点之后才接入尾段均值。以秒为单位的主边界为
...
**图4（第二问独立稳定分界与敏感性，已生成）：** figures/q2/边界独立分界图.png、figures/q2/02_endpoint_comparison.png 和 figures/q2/04_model_sensitivity.png。图中突出：完整数据拟合曲线在观测区间连续，温度在 5280 s、环境水分在 6780 s 分别平滑接入稳定尾段，且后续结果对传质边界相比导热系数更敏感。

![图4a：第二问温度与环境水分的独立稳定分界](figures/q2/边界独立分界图.png)
![图4b：拟合终点比较](figures/q2/02_endpoint_comparison.png)
![图4c：第二问参数敏感性](figures/q2/04_model_sensitivity.png)
...
**图5（第二问结果图，已生成）：** figures/q2/第二问结果图.png。左右两个面板分别给出温度场和水分浓度场，横轴为时间、纵轴为距中心距离，沿用图3的温度深蓝—深红和水分浓度灰—蓝配色；径向绘图采样为 0.01 cm，突出“温度先均匀、水分后均匀”的时间尺度差异。

![图5：第二问全过程温度与水分浓度场](figures/q2/第二问结果图.png)
...
**图6（第三问结果图，已生成）：** figures/q3/第三问结果图.png。左右两个面板分别给出温度场和水分浓度场，沿用图3配色，径向绘图采样为 0.01 cm；水分浓度场叠加 0.15 阈值等值线和报告结束时刻，说明全域事件由中心控制。

![图6：第三问全域干燥终点温度与水分浓度场](figures/q3/第三问结果图.png)
...
**图7（第四问收缩与径向分布，已生成）：** figures/q4/第四问结果图.png。左、中两个面板分别给出沿用图3配色的温度场和水分浓度场，按 0.01 cm 的实际距离采样；黑色实线表示当前药材表面，表面至 2.0 cm 的收缩外部区域以均匀斜线遮罩，避免将药材外部误读为低水分。右图保留半径观测值与 PCHIP 插值曲线。

![图7：第四问收缩模型与温度、水分浓度场](figures/q4/第四问结果图.png)

**图8（半径插值比较，已生成）：** figures/q4/03_radius_methods.png。图中同时呈现观测点、线性、PCHIP、Akima 和三次样条，突出高阶平滑方法虽可能 RMSE 较小，却产生了物理上不允许的局部增大。

![图8：半径插值方法比较](figures/q4/03_radius_methods.png)
