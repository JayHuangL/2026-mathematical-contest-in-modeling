...
**问题二：分界点后的连续边界。** 
本文中拟合方式使用经比较更优的拉伸指数函数，温度和环境水分采用各自不同的拟合窗口：温度拟合窗口为 \(0\)--\(4980\ \mathrm s\)，环境水分拟合窗口为 \(0\)--\(6480\ \mathrm s\)。得到的分界前拟合为
...
分界前拟合 RMSE 分别为 \(0.12769797\,^\circ\mathrm C\)（温度，0--4980 s）和 \(1.80280\times10^{-4}\ \mathrm{kg/kg}\)（环境水分，0--6480 s）。

为保证函数平滑过渡，本文由稳定分界点向两侧各取 300 s，建立 600 s 平滑过渡区。记过渡区左、右端点为“过渡点 1”和“过渡点 2”，则
...
\(\lambda_y(t)=(t-t_{-,y})/600\) 做线性平滑连接；过渡点 2 以后使用相应稳定段观测的算术均值。具体地，温度边界的过渡区为 [4980,5580] s，环境水分边界的过渡区为 [6480,7080] s，尾段均值分别为 \(T_{a,*}=49.88579739\,^\circ\mathrm C\) 和 \(C_{a,*}=0.04982109\ \mathrm{kg/kg}\)。
