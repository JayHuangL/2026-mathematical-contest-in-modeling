...
主结果为：第一问 1800 s 时体积平均温度 35.169887 °C、平均干基含水率 2.293557 kg/kg；第二问 3 h 时平均温度 49.8096 °C、平均含水率 1.3822 kg/kg；第三问固定半径模型的全域达标时间为 57.6777 h；第四问采用附录4和 PCHIP 半径后，达标时间为 51.2673 h。网格、时间积分、Jacobian、含水率物理范围和水分收支检查表明数值误差已收敛，但长期边界延拓、有效传质系数、物性经验式和均匀收缩假设带来的不确定性仍明显大于离散误差。因此，文中时间均解释为给定输入和假设下的模型预测，而非精确到秒的实验结论。
...
对任一环境量 \(y(t)\)，固定 \(t=0\) 的观测初值 \(y_0\)，采用初值固定的拉伸指数函数
...
其中 \(t\) 和时间尺度 \(\tau\) 用秒计，\(A\) 为与 \(y\) 同量纲的幅值，\(p\) 为无量纲形状参数，\(\widehat y\) 表示模型预测值。参数通过非线性最小二乘（最小化预测值与观测值残差平方和）得到。对候选方法采用按时间顺序的时间块留出验证（time-ordered blocked hold-out），不随机打乱时间记录，而是按预定时间间隔留出一组时间点作为验证块；这里的“阻塞验证 RMSE”就是这些留出时间点上的均方根误差。若验证样本为 \((t_i,y_i)\)，则
...
RMSE 衡量预测误差，其中 \(n\) 是验证样本数。为比较曲线的局部弯折程度，另在验证区间内取 721 个等间距点，定义二阶差分粗糙度
...
其中 \(N_d=721\)，\(\Delta t\) 是这 721 个等间距点的时间步长。该指标是平均绝对二阶差分，数值越小表示曲线越平滑；温度和含水率的粗糙度因量纲不同，只在同一序列内比较。第一问的候选方法比较和拉伸指数参数估计均只使用 0--1800 s 数据，每隔 5 个记录留出 1 个样本（25 个训练点、6 个验证点），候选结果为：
...
候选方法的含义如下：分段线性是在相邻观测点之间作直线连接；PCHIP 是保持数据形状的分段三次 Hermite 插值；Akima 是按局部斜率加权的分段插值；自然三次样条是二阶导数在两端为零的三次样条；平滑样条则在拟合残差与曲率惩罚之间折中，其中曲率惩罚用于抑制曲线二阶导数过大。温度序列中拉伸指数模型的阻塞验证（时间留出）RMSE 最低。含水率序列中自然三次样条的 RMSE 为 \(4.13\times10^{-5}\)，低于拉伸指数模型的 \(5.54\times10^{-5}\)，但其粗糙度约为拉伸指数模型的 69.5 倍；平滑样条的粗糙度略低于拉伸指数模型。综合 RMSE、粗糙度、平滑性以及同一拉伸指数形式对两类边界的统一表示，第一问选择初值固定的拉伸指数模型。拟合参数为
...
第一问只使用附件1的 0--1800 s 观测数据估计式(3)的参数；在全部 0--1800 s 观测点上计算的主拟合 RMSE 分别为 0.104306 °C 和 \(6.73349\times10^{-5}\ \mathrm{kg/kg}\)。求解时在同一 0--1800 s 区间内调用该连续边界。为衡量窗口选择的不确定性，另计算 3600 s 和 7200 s 扩展窗口情景，但它们不替代第一问的主方案。不同终点的比较见 results/q1/boundary_endpoint_comparison.csv。

**图1（边界方法与早期曲线，已生成）：** figures/q1/01_boundary_methods_and_phase.png。图中展示原始 60 s 观测点、六类候选曲线及其 0--1800 s 早期段；读图重点是拉伸指数模型的平滑性和曲线形态，而不是只比较单个 RMSE。
...
**图2（第一问径向场，已生成）：** figures/q1/第一问结果图.png。四个面板分别展示温度/含水率随时间变化及不同时间径向剖面；图中最重要的视觉结论是温度从中心向表面升高、含水率从中心向表面降低。
...
第二问及其后的第三、第四问的环境边界使用与第一问相同的式(1)--(2)和同一组函数族，但拟合窗口覆盖附件1的完整 0--14400 s。这里的“全窗口拟合 RMSE”是指在全部 0--14400 s 观测点上计算的拟合残差 RMSE，不是第一问的时间留出 RMSE。分界点之前直接使用这一全数据拟合边界；只有在稳定分界点之后才接入尾段均值。以秒为单位的主边界为
...
全窗口 RMSE 分别为 \(0.1441934\,^\circ\mathrm C\) 和 \(1.81827\times10^{-4}\ \mathrm{kg/kg}\)。第二问的主体拟合曲线因此覆盖 0--14400 s，而结果工作簿取 0--10800 s 的每秒输出。

稳定点识别先对原始序列施加 11 点 Savitzky--Golay 平滑；该方法在局部窗口内用低阶多项式拟合，以降低测量噪声而尽量保留趋势。随后对每个序列在连续的 1800 s 窗口上同时计算极差和线性拟合斜率。对温度序列，连续三个窗口均满足极差不超过 \(0.72\,^\circ\mathrm C\)、斜率绝对值不超过 \(4.0\times10^{-4}\,^\circ\mathrm C/\mathrm s\) 时，将第一个窗口起点定义为温度稳定点；对环境水分序列，分别使用极差不超过 \(8.6\times10^{-4}\ \mathrm{kg/kg}\)、斜率绝对值不超过 \(4.78\times10^{-7}\ \mathrm{(kg/kg)/s}\) 的条件。由此得到温度稳定点 5280 s、环境水分稳定点 6780 s。两者相差 1500 s，不能用一个共同分界点代替；分别为温度和水分设置 600 s 的平滑过渡区：
...
温度边界在 [4980,5580] s 内、环境水分边界在 [6480,7080] s 内分别由拟合值线性过渡到各自尾段均值 \(T_{a,*}=49.88579739\,^\circ\mathrm C\)、\(C_{a,*}=0.04982109\ \mathrm{kg/kg}\)。尾段均值是相应稳定判定区间之后所保留观测段的算术平均值；过渡区之间仍保留各物理量自身的拟合/稳定值。这一步是边界输入的连续化，不重置药材内部状态。
...
**图3（第二问独立稳定分界与敏感性，已生成）：** figures/q2/边界独立分界图.png、figures/q2/02_endpoint_comparison.png 和 figures/q2/04_model_sensitivity.png。图中突出：完整数据拟合曲线在观测区间连续，温度在 5280 s、环境水分在 6780 s 分别平滑接入稳定尾段，且后续结果对传质边界相比导热系数更敏感。
...
**图4（第二问结果图，已生成）：** figures/q2/第二问结果图.png。图中同时给出中心/表面随时间变化和不同时间的径向剖面，突出“温度先均匀、水分后均匀”的时间尺度差异。
...
第三问沿用第二问的独立边界处理和附录3物性：温度在 4980--5580 s 内由全 0--14400 s 拉伸指数拟合值过渡到 49.88579739 °C，环境水分在 6480--7080 s 内由同一拟合值过渡到 0.04982109 kg/kg；两种边界分别在自己的稳定点后保持尾段均值。后者是一项长期外推假设，后文单独给出敏感性。固定半径为 \(R=0.02\ \mathrm m\)，初始条件和 Robin 边界仍为式(6)、式(13)，有限体积通量和面积权重仍为式(8)、式(14)。
...
**图5（第三问结果图，已生成）：** figures/q3/第三问结果图.png。图中标出 0.15 阈值、中心与表面时间曲线，以及结束时刻的径向剖面，说明全域事件由中心控制。
...
使用全域事件 \(M(t)=\max_i U_i(t)\) 和式(16)的严格阈值，得到未舍入事件时刻 \(184562.003657\ \mathrm s=51.2672232379\ \mathrm h\)。向上保留四位小数并继续积分后，报告烘干时间为 **51.2673 h**；报告时刻半径 1.2000 cm，全域最大含水率 0.149999858678 kg/kg，平均含水率 0.1183955958 kg/kg。
...
**图6（第四问收缩与径向分布，已生成）：** figures/q4/第四问结果图.png。左图显示半径随时间下降，中图显示中心、内部点和表面的失水曲线，右图显示收缩后实际距离坐标下的径向分布；固定实际位置越过表面时显示为空白。

![图6：第四问收缩模型与径向分布](figures/q4/第四问结果图.png)

**图7（半径插值比较，已生成）：** figures/q4/03_radius_methods.png。图中同时呈现观测点、线性、PCHIP、Akima 和三次样条，突出高阶平滑方法虽可能 RMSE 较小，却产生了物理上不允许的局部增大。
