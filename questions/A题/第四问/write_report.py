...
grid=['| 区间数 | 阈值时间 / h | 与上一网格相差 / s | 有效含水率最大差 |','|---:|---:|---:|---:|']
...
\[
t_{\mathrm{结束}}={{finish_s}}\ \mathrm s={{finish_h}}\ \mathrm h.
\]

此时最大含水率{{final_C}} kg/kg，平均含水率{{mean_C}} kg/kg。中心四位小数显示0.1500，但严格判定采用未舍入值。
...
最后两级网格结束时间相差{{grid_diff}} s。比较时核对相同分钟时刻、相同实际位置及越界空白掩码。

### 时间精度与方程检查

收紧容差10倍、后4 h最大步长减为120 s，阈值时间差{{time_diff}} s，共同有效含水率最大差{{time_C}} kg/kg。

- 解析Jacobian复步长检查最大相对差：{{jac}}。
- 固定半径并换回附录3，与第二问方程右端最大差：{{fixed}}。
- 无外部驱动力时，均匀温度与含水率在收缩过程中导数为0。
- 式(9)累计收支残差：{{balance}} kg/kg。
...
另两项敏感性对照：
- 半径使用分段线性插值：{{linear_h}} h，相对同网格PCHIP变化{{linear_delta}} h。
- 4 h后环境用50°C、0.05：{{tail_h}} h，相对同网格基准变化{{tail_delta}} h。
...
工作簿共{{rows}}个时间行，越界空白{{blank}}个，表面列全程有值。导出后重新读取，逐项核对数值、半径约束、空白位置和四位小数格式，检查两张附件未改动。
"""
...
'linear_h':f"{cases['linear_radius']:.6f}",'linear_delta':f"{cases['linear_radius']-baseline:+.6f}",
'tail_h':f"{cases['tail_50C_0.05']:.6f}",'tail_delta':f"{cases['tail_50C_0.05']-baseline:+.6f}",
'rows':str(s['output_rows']),'blank':str(s['outside_blank_count'])}
for k,v in values.items():report=report.replace('{{'+k+'}}',v)
report=report.replace('FENCE',chr(96)*3)
assert '{{' not in report
(HERE/'第四问公式与求解.md').write_text(report,encoding='utf-8')
