...
const root=arg('--root',path.dirname(here));
const output=arg('--output-dir',path.join(root,'outputs','q4'));
const previews=path.join(here,'previews');
await fs.mkdir(previews,{recursive:true});
const wb=await SpreadsheetFile.importXlsx(await FileBlob.load(path.join(root,'附件','附件3','result4.xlsx')));
...
if(process.argv.includes('--preview')){
 ...
 const data=JSON.parse(await fs.readFile(path.join(here,'result4_data.json'),'utf8'));
 const last=data.t_s.length+1;
 sheet.getRange('A1:V1').values=[['时间\\到药材中心的距离',...data.fixed_r_cm,'药材表面']];
 sheet.getRange(`A2:V${last}`).values=data.t_s.map((t,i)=>[t,...data.C[i]]);
 const range=sheet.getRange(`A1:V${last}`);
 range.format.font={name:'宋体',size:10};
 range.format.horizontalAlignment='center';
 range.format.verticalAlignment='center';
 range.format.rowHeight=16;
 ...
 await file.save(path.join(output,'result4.xlsx'));
 console.log('Saved result4.xlsx');
}
process.exit(0);
