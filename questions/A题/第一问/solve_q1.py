...
def fit_environment(environment, fit_window_s):
    ...
    diagnostics = {'family': 'initial-value-fixed stretched exponential',
...
def write_sensitivity_report(sensitivity):
    ...
    lines = [
        ...
        '## 1. 结论与使用范围',
        '',
        f"第一问主计算采用附件 1 的 0–{int(fit['fit_window_s'])} s 数据，拟合初值固定的拉伸指数函数；求解器仅在 0–1800 s 内直接调用该函数作为边界条件。它用于平滑 60 s 记录间的输入，不改变第一问的控制方程、初值和材料参数。",
        '',
