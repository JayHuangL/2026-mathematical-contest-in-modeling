...
Input: 附件/附件1.xlsx. Excel export: build_workbook.mjs.
"""
...
def main():
    ...
    q1_path = ROOT/'第一问/result1_full_precision.npz'
    if q1_path.exists():
        with np.load(q1_path) as q1:
            summary['q1_vs_q2_at_1800'] = {key: {'q1': q1[key][-1, [0, 20]].tolist(),
                                                           'q2': result[key][1800, [0, 20]].tolist()}
                                                 for key in ['T', 'C']}
