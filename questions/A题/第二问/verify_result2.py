...
assert hashlib.sha256((ROOT/'附件/附件1.xlsx').read_bytes()).hexdigest() == summary['input_sha256']
data = json.loads((HERE/'result2_data.json').read_text(encoding='utf-8'))
book = openpyxl.load_workbook(ROOT/'outputs/q2/result2.xlsx', read_only=True, data_only=True)
...
book.close()
report = (HERE/'第二问公式与求解.md').read_text(encoding='utf-8')
