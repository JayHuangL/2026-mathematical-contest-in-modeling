...
sens=['| 4 h后的环境条件 | 温度 / °C | 环境水分浓度 / (kg/kg) | 阈值时间 / h | 相对同网格基准的变化 / h |','|---|---:|---:|---:|---:|']
base=next(r for r in s['grid_checks'] if r['N']==800)
sens.append(f"| 末值保持（基准） | {s['tail_T']:.6f} | {s['tail_C']:.8f} | {base['event_s']/3600:.6f} | 0 |")
for r in s['tail_sensitivity']:
    label='50°C、0.05' if r['case']=='50C_0.05' else '最后1 h记录均值'
    sens.append(f"| {label} | {r['T_tail']:.6f} | {r['C_tail']:.8f} | {r['event_h']:.6f} | {r['delta_h_at_N800']:.6f} |")
report=r"""# 第三问：烘干结束时间与全过程水分分布

采用第二问的控制方程、附录3物性及边界假设，计算得到阈值穿越时间约为 **{{event_h}} h**。按四位小数小时向上取整后，给出满足严格“低于0.15”的烘干时长：
...
非均匀网格加密后，最后两级的结束时间差为{{grid_last_diff}} s。先前均匀3200区间得到约57.1688 h，表面加密后结果约57.1694 h；这说明有必要直接检查结束时间的收敛，而不只看结果表是否大致相同。
